...
modern_prlog_lm = lm(ModernC ~ ., data = UN3_log[,c(-3,-5,-10)]) #lm w transformed PPgdp,Pop
#summary(modern_prlog_lm)
boxCox(modern_prlog_lm)
lambda_y = powerTransform(modern_prlog_lm)$lambda
```
...
modern_lm2 = lm(ModernC ~ ., data = UN3_log[,c(-3,-5,-10)])
summary(modern_lm2)
par(mfrow=c(2,2))
...
There are outliers but no significant influential points in the data. In the original untransformed data, China and India looked as potential influential points due to their high leverage in the residual plots. However, Cook and we had Cook's Island. However, after we transform the data
